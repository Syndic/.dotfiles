...
def fake_art_dir(tmp_path, monkeypatch):
    ...
    art_dir = tmp_path / "ascii_art" / phase2.ASCII_ART_TRUECOLOR_SUBDIR
...
def test_picks_largest_that_fits(fake_art_dir, monkeypatch, capsys):
    ...
    phase2.display_kilobyte(phase2.ASCII_ART_TRUECOLOR_SUBDIR)
...
def test_falls_back_to_smallest_when_nothing_fits(fake_art_dir, monkeypatch, capsys):
    ...
    (fake_art_dir / "kilobyte40.txt").write_text("X" * 40 + "\n")
    (fake_art_dir / "kilobyte80.txt").write_text("Y" * 80 + "\n")
    (fake_art_dir / "kilobyte160.txt").write_text("Z" * 160 + "\n")
    set_terminal_cols(monkeypatch, 10)
    phase2.display_kilobyte(phase2.ASCII_ART_TRUECOLOR_SUBDIR)
    out = capsys.readouterr().out
    assert "X" * 40 in out
    assert "Y" * 80 not in out
    assert "Z" * 160 not in out
...
def test_no_output_when_art_dir_empty(tmp_path, monkeypatch, capsys):
    (tmp_path / "ascii_art" / phase2.ASCII_ART_TRUECOLOR_SUBDIR).mkdir(parents=True)
    monkeypatch.setattr(phase2, "DOTFILES_DIR", tmp_path)
    set_terminal_cols(monkeypatch, 100)
    phase2.display_kilobyte(phase2.ASCII_ART_TRUECOLOR_SUBDIR)
    assert capsys.readouterr().out == ""
...
def test_uses_requested_format_subdir(tmp_path, monkeypatch, capsys):
    truecolor_dir = tmp_path / "ascii_art" / phase2.ASCII_ART_TRUECOLOR_SUBDIR
    fallback_dir = tmp_path / "ascii_art" / phase2.ASCII_ART_256_COLOR_SUBDIR
    truecolor_dir.mkdir(parents=True)
    fallback_dir.mkdir(parents=True)
    (truecolor_dir / "kilobyte80.txt").write_text("TRUECOLOR\n")
    (fallback_dir / "kilobyte80.txt").write_text("FALLBACK\n")
    monkeypatch.setattr(phase2, "DOTFILES_DIR", tmp_path)
    set_terminal_cols(monkeypatch, 100)

    phase2.display_kilobyte(phase2.ASCII_ART_256_COLOR_SUBDIR)

    out = capsys.readouterr().out
    assert "FALLBACK" in out
    assert "TRUECOLOR" not in out
